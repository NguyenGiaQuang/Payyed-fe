...
import { logout as logoutApi } from "../../api/auth";     // ⬅️ sửa đường dẫn
import { clearAuth } from "../../utils/auth";            // ⬅️ sửa đường dẫn
...
const AdminHeader = ({ active = "admin" }) => {
    ...
    // ⬇️ Sửa lại hàm logout cho đúng
    const handleLogout = async (e) => {
        e.preventDefault();
        try {
            await logoutApi();   // POST /api/auth/logout -> clear cookie trên backend
        } catch (err) {
            console.error("Logout failed", err);
        } finally {
            clearAuth();         // xoá trạng thái login ở localStorage
            navigate("/login");
        }
    };
                            ...
                            <div id="header-nav" className="collapse navbar-collapse show">
                                <ul className="navbar-nav me-auto">
                                    ...
                                    <li className={active === "balances" ? "active" : ""}>
                                        <NavLink to="/admin/system-balance" className="nav-link">
                                            Tổng hợp số dư
                                        </NavLink>
                                    </li>
                                </ul>
                        ...
                        <nav className="login-signup navbar navbar-expand">
                            <ul className="navbar-nav">
                                <li className={`dropdown language ${langOpen ? "show" : ""}`}>
                                    <a href="#!" className="dropdown-toggle" onClick={toggleLang}>
                                        VN
                                    </a>
